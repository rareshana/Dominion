...
class TestStarter1(unittest.TestCase): #カードをサプライから購入した際の挙動を確認
	def setUp(self):
		print('setUp')
	...
	def test_gain_copper(self): #銅貨を購入する
		...
		self.assertEaual(self._game.player[0].coins, 3) #残りは3金
		self.assertEaual(self._game.player[0].restbuys, 0) #残り購入権は0
	...
	def test_gain_estate(self): #屋敷を購入する
		...
		self.assertEaual(self._game.player[0].coins, 1) #残りは3金
		self.assertEaual(self._game.player[0].restbuys, 0) #残り購入権は0